...
export default class DissolveEffect extends ParticleContainer {
  ...
  constructor(private sourceSprite: Sprite, options: IDissolveEffectOptions = {}) {
  ...
  private prepare(): void {
    ...
    for (let py = 0; py < height; py += pixelSize) {
      for (let px = 0; px < width; px += pixelSize) {
        ...
        let spatialFactor = 0;
        switch (direction) {
          case 'left-to-right': spatialFactor = px / width; break;
          case 'right-to-left': spatialFactor = 1 - (px / width); break;
          case 'top-to-bottom': spatialFactor = py / height; break;
          case 'bottom-to-top': spatialFactor = 1 - (py / height); break;
          case 'center-out':
            const dx = px - width / 2;
