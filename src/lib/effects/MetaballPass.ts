...
export interface IMetaballPassOptions {
  ...
  clearColor?: number | string | number[]
...
export default class MetaballPass extends Container {
  ...
  private readonly _blur: InstanceType<typeof filters.BlurFilter>
  ...
  constructor(options: IMetaballPassOptions) {
    ...
    this._blur = new filters.BlurFilter(this._options.blurStrength, 4, undefined, 5)
    this._thresholdFilter = new Filter(undefined, thresholdSmoothFrag, {
      uThreshold: this._options.threshold,
      uSoftness: this._options.edgeSoftness,
    })
    ...
    if (this._options.autoUpdate) {
      ...
      this._ticker = ticker
      this._tickerCb = () => {
        if (!this._isDestroyed) this.updateCapture()
      }
  ...
  updateCapture(): void {
    ...
    this._rendererRef.render(this.source, {
